...
with st.sidebar:
    st.header("Зареждане на данни")
    file_ei_ed = st.file_uploader("Качете файл с Ei/Ed изолинии", type=["xlsx"])
    file_esr_ei = st.file_uploader("Качете файл с Esr/Ei изолинии", type=["xlsx"])

if file_ei_ed and file_esr_ei:
    df_ei_ed = pd.read_excel(file_ei_ed)
    df_esr_ei = pd.read_excel(file_esr_ei)
    ...
    if all(v > 0 for v in h_list) and all(v > 0 for v in E_list):
        ...
        if Ei > 0 and Ed > 0 and D > 0:
            ...
            fig = px.scatter()
            fig.add_scatter(x=df_ei_ed['Easting (x) H/D'], y=df_ei_ed['Easting (x) σR'],
                            mode='markers', name='Ei/Ed точки', marker=dict(color='blue'))
            fig.add_scatter(x=df_esr_ei['Easting (x) H/D'], y=df_esr_ei['Easting (x) σR'],
    ...
    st.info("Моля, качете и двата Excel файла.")
