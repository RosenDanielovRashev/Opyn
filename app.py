...
# Session state за управление на текущия пласт
if 'current_layer' not in st.session_state:
    st.session_state.current_layer = n

# Бутони за навигация между пластовете
col1, col2, col3 = st.columns(3)
with col1:
    if st.button("← Предишен пласт") and st.session_state.current_layer > 1:
        st.session_state.current_layer -= 1
with col2:
    if st.button("Следващ пласт →") and st.session_state.current_layer < n:
        st.session_state.current_layer += 1
with col3:
    calculate_without_first_two = st.button("Изчисли без първите два пласта")

# Въвеждане на h_i и E_i за всеки пласт
st.markdown(f"### Въведи стойности за всеки пласт (Текущ пласт: {st.session_state.current_layer})")
h_values = []
E_values = []
cols = st.columns(2)
for i in range(n):
...
def perform_calculations(h_array, E_array, n_layers, title_suffix=""):
    # Изчисляване на Esr за първите n_layers-1 пласта
    sum_h_n_1 = h_array[:n_layers-1].sum()
    weighted_sum_n_1 = np.sum(E_array[:n_layers-1] * h_array[:n_layers-1])
    Esr = weighted_sum_n_1 / sum_h_n_1 if sum_h_n_1 != 0 else 0

    # Изчисляване на H_n и H_{n-1}
    H_n = h_array[:n_layers].sum()
    H_n_1 = sum_h_n_1
    ...
    st.markdown(f"### Изчисления {title_suffix}")
    st.latex(r"H_{n-1} = \sum_{i=1}^{n-1} h_i")
    h_terms = " + ".join([f"h_{to_subscript(i+1)}" for i in range(n_layers-1)])
    st.latex(r"H_{n-1} = " + h_terms)
    st.write(f"H{to_subscript(n_layers-1)} = {H_n_1_r}")

    st.latex(r"H_n = \sum_{i=1}^n h_i")
    h_terms_n = " + ".join([f"h_{to_subscript(i+1)}" for i in range(n_layers)])
    st.latex(r"H_n = " + h_terms_n)
    st.write(f"H{to_subscript(n_layers)} = {H_n_r}")

    st.latex(r"Esr = \frac{\sum_{i=1}^{n-1} (E_i \cdot h_i)}{\sum_{i=1}^{n-1} h_i}")

    numerator = " + ".join([f"{round(E_values[i],3)} \cdot {round(h_values[i],3)}" for i in range(n_layers-1)])
    denominator = " + ".join([f"{round(h_values[i],3)}" for i in range(n_layers-1)])
    ...
    En = E_values[n_layers-1]
    En_r = round(En, 3)

    st.markdown("### Изчисления с последен пласт")

    st.latex(r"E_{" + str(n_layers) + r"} = " + f"{En_r}")

    Esr_over_En = Esr / En if En != 0 else 0
    Esr_over_En_r = round(Esr_over_En, 3)
    st.latex(r"\frac{Esr}{E_{" + str(n_layers) + r"}} = \frac{" + f"{Esr_r}" + "}{" + f"{En_r}" + "} = " + f"{Esr_over_En_r}")

    En_over_Ed = En / Ed if Ed != 0 else 0
    En_over_Ed_r = round(En_over_Ed, 3)
    st.latex(r"\frac{E_{" + str(n_layers) + r"}}{E_d} = \frac{" + f"{En_r}" + "}{" + f"{Ed_r}" + "} = " + f"{En_over_Ed_r}")

    return H_n_r, ratio_r, Esr_over_En_r, En_over_Ed_r
...
if st.button("Покажи графиката"):
    ...
    # Основно изчисление за всички пластове
    H_n_r, ratio_r, Esr_over_En_r, En_over_Ed_r = perform_calculations(h_array, E_array, n, "(за всички пластове)")
    
    # Допълнително изчисление без първите два пласта ако е натиснат бутона
    if calculate_without_first_two and n > 2:
        st.markdown("---")
        perform_calculations(h_array[2:], E_array[2:], n-2, "(без първите два пласта)")
    
    # Изчисление за текущия избран пласт
    if st.session_state.current_layer != n:
        st.markdown("---")
        perform_calculations(h_array[:st.session_state.current_layer], 
                           E_array[:st.session_state.current_layer], 
                           st.session_state.current_layer, 
                           f"(за пласт {st.session_state.current_layer})")
    ...
    try:
        ...
        st.error("❌ Липсват необходимите CSV файлове за визуализация.")
